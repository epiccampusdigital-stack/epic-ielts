...
async function main() {
  ...
  try {
    ...
    // SEED READING 003 PAPER & QUESTIONS
    console.log('Checking READING 003...');
    let paper = await prisma.paper.findFirst({ where: { paperCode: '003', testType: 'READING' } });
    if (!paper) {
      ...
      // ... (Questions seeding could go here if needed, but it should already be there from previous push)
    }
    ...
    for (const passage of passages) {
      const existingPassage = await prisma.passage.findFirst({
        where: { paperId: paper.id, passageNumber: passage.passageNumber }
      });
      if (existingPassage) {
        ...
        await prisma.passage.create({
          data: { paperId: paper.id, passageNumber: passage.passageNumber, title: passage.title, text: passage.text }
        });
      }
